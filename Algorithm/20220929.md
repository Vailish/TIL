...
for _ in range(m):
    ...
    if x_root != y_root:  # 서로소 집합인 경우만 합집합 연산
        if x_root < y_root:  # 편의, 안해도 되는데 작은거에 붙이는게 보기 좋아서 써는것
...
for dist, x, y in edges:
    ...
    if x_root != y_root:  # 사이클이 아니면 == 서로소가 아니면 == 대표노드가 다르면
        parent[y_root] = x_root  # union
        cost += dist
        counts += 1

        if counts >= n - 1:  # 간선의 최대 개수는 정점 - 1 이므로 break // 가지치기
            break
